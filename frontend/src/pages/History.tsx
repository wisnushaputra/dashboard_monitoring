...
export default function History() {
  ...
  const chartData = data?.events?.map((e: any) => ({
    time: new Date(e.timestamp).toLocaleString('id-ID'),
      ...
      {data && (
        <>
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
            {[
              { label: 'Total Alarms', value: data.stats.totalAlarms },
              { label: 'Total Downtime', value: `${data.stats.totalDowntime}s` },
              { label: 'Availability', value: `${data.stats.availability}%` },
            ...
            <div className="divide-y text-sm max-h-64 overflow-y-auto">
              {data.events.map((e: any) => (
                <div key={e.id} className="px-4 py-2 flex items-center gap-3">
                  ...
                  <span className="text-zinc-500 text-xs">{new Date(e.timestamp).toLocaleString('id-ID')}</span>
